...
def fit_hier_params(model_matrix, sample_kwargs=None):
    ...
    if sample_kwargs is None:
        ...
        # draws = 5000, tune = 5000, init = 'advi+adapt_diag'

    with pm.Model() as hier_model:
        mu_1 = 0. #pm.Normal('mu_mean', mu=0., sd=100.)
        mu_2 = 0. #pm.Normal('mu_uncertainity', mu=0., sd=100.)
        mu_3 = 0. #pm.Normal('mu_stick', mu=0., sd=100.)

        sigma_1 = 5. #pm.HalfCauchy('sigma_mean', beta=100)
        sigma_2 = 5. #pm.HalfCauchy('sigma_uncertainity', beta=100)
        sigma_3 = 5. #pm.HalfCauchy('sigma_stick', beta=100)
        ...
        # inference!
        trace = pm.sample_smc(**sample_kwargs) #1000, random_seed=42)
...
def construct_sticky_choice(pred_data, n_arms=6):
    ...
    # so, not tnevery one uses every response, so we need to correct for this
...
# def construct_corner_choice(pred_data, n_arms=6):
#     x_sc = []
#     y = pd.get_dummies(pred_data.loc[:, 'prev_arm'])
#     # so, not tnevery one uses every response, so we need to correct for this
#     for c in set(range(0, n_arms)):
...
def fit_models(model_name, sample_kwargs, analysis_name, subject, rule, experiment, fit_tasks, fit_subtasks, fit_trials, seed):
    ...
    print("Fitting bayesian softmax policy for predictions from {} from the {} experiment: participant {}".format(model_name, experiment, subject))
    ...
    #x_corner = construct_corner_choice(pred_data, NUM_ARMS) 
...
if __name__ == '__main__':
    ...
    MODEL_PATH = f'/notebooks/scratch/modelfits/baselines_to_participants/{rule}/'#.format()
