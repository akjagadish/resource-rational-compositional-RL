...
parser = argparse.ArgumentParser(description='Fit  meantracker')
    ...
    for per in ['perodd', 'pereven']:
        for init_idx in range(n_inits):
            ...
            for run in range(n_runs):
                if rule == 'add':
                    print('use mean_tracker for additive rule')
                    model = MeanTracker(num_iters)
                else:
                    print('use mean_tracker model for cp rule')
                    model = MeanTracker(num_iters)
...
np.save('{}/regrets_mean_tracker_simulated_greedy_{}_{}_{}'.format(path, condition, rule, eps), regrets)
np.save('{}/actions_mean_tracker_greedy_{}_{}_{}'.format(path, condition, rule, eps), actions)